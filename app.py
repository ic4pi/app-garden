...
class LLMClient:
            ...
            try:
                async with httpx.AsyncClient(timeout=Config.REQUEST_TIMEOUT) as client:
                    ...
                    if response.status_code == 404:
                        return f"ERROR: NVIDIA model '{model}' not found (404)"
    ...
    async def call_openrouter(self, messages, model=None, temperature=0.7, max_tokens=4000):
        async with self.semaphore:
            ...
            use_fallback_model = model != Config.FALLBACK_RANKER
            try:
                async with httpx.AsyncClient(timeout=Config.REQUEST_TIMEOUT) as client:
                    while retry_count <= Config.OPENROUTER_RETRY_COUNT:
                        ...
                        if response.status_code in (429, 402):
                            ...
                            next_key = self.api_keys.get_next_key('openrouter', paid_fallback=False)
                            if next_key:
                                key = next_key
                                ...
                                delay *= 2
                                continue
                            paid_key = self.api_keys.get_next_key('openrouter', paid_fallback=True)
                            if paid_key:
                                key = paid_key
                                headers["Authorization"] = f"Bearer {key}"
                                await asyncio.sleep(delay)
                        ...
                        if response.status_code == 404:
                            ...
                            if use_fallback_model:
                                ...
                                delay *= 2
                                continue
                            return f"ERROR: OpenRouter model '{model}' not found (404)"
                        response.raise_for_status()
                        data = response.json()
